    ...
    if (get_magic_quotes_gpc())
    {
        while (list($z, $X) = each($ng))
        {
            foreach ($X as $be => $W)
            {
                ...
                else $ng[$z][stripslashes($be) ] = ($Tc ? $W : stripslashes($W));
            }
        }
    }
}
function bracket_escape($v, $Na = false)
{
    ...
    return strtr($v, ($Na ? array_flip($qi) : $qi));
}
...
function optionlist($vf, $ch = null, $Ni = false)
{
    ...
    foreach ($vf as $be => $W)
    {
        ...
        foreach ($wf as $z => $X) $I .= '<option' . ($Ni || is_string($z) ? ' value="' . h($z) . '"' : '') . (($Ni || is_string($z) ? (string)$z : $X) === $ch ? ' selected' : '') . '>' . h($X);
...
{
    if ($of) return "<select name='" . h($C) . "'" . ($je ? " aria-labelledby='$je'" : "") . ">" . optionlist($vf, $Y) . "</select>" . (is_string($of) ? script("qsl('select').onchange = function () { $of };", "") : "");
    $I = "";
    foreach ($vf as $z => $X) $I .= "<label><input type='radio' name='" . h($C) . "' value='" . h($z) . "'" . ($z == $Y ? " checked" : "") . ">" . h($X) . "</label>";
    return $I;
}
...
{
    return script("$dh.onclick = function () { return confirm('" . ($Je ? js_escape($Je) : 'Are you sure?') . "'); };", "");
}
...
{
    $_SESSION["pwds"][$Ui][$N][$V] = ($_COOKIE["adminer_key"] && is_string($F) ? array(
...
{
    global $ba;
    return header("Set-Cookie: $C=" . urlencode($Y) . ($te ? "; expires=" . gmdate("D, d M Y H:i:s", time() + $te) . " GMT" : "") . "; path=" . preg_replace('~\?.*~', '', $_SERVER["REQUEST_URI"]) . ($ba ? "; secure" : "") . "; HttpOnly; SameSite=lax", false);
}
...
{
    if (!ini_bool("session.use_cookies") || ($Zc && @ini_set("session.use_cookies", false) !== false)) session_write_close();
}
...
function query_redirect($G, $ve, $Je, $zg = true, $Ac = true, $Lc = false, $ci = "")
{
    ...
    $wh = "";
    if ($G) $wh = $b->messageQuery($G, $ci, $Lc);
    if ($Lc)
    {
        $o = error() . $wh . script("messagesPrint();");
        return false;
    }
    if ($zg) redirect($ve, $Je . $wh);
    return true;
}
function queries($G)
{
...
function apply_queries($G, $S, $xc = 'table')
{
    foreach ($S as $Q)
    {
        if (!queries("$G " . $xc($Q))) return false;
    }
    return true;
}
function queries_redirect($ve, $Je, $zg)
{
    ...
    return query_redirect($sg, $ve, $Je, $zg, false, !$zg, $ci);
}
function format_time($Ah)
{
    return sprintf('%.3f s', max(0, microtime(true) - $Ah));
}
function remove_from_uri($Kf = "")
{
    return substr(preg_replace("~(?<=[?&])($Kf" . (SID ? "" : "|" . session_name()) . ")=[^&]*&~", '', "$_SERVER[REQUEST_URI]&") , 0, -1);
}
...
function get_file($z, $Pb = false)
{
    ...
    foreach ($Rc["error"] as $z => $o)
    {
        ...
        {
            $Ah = substr($yb, 0, 3);
            if (function_exists("iconv") && preg_match("~^\xFE\xFF|^\xFF\xFE~", $Ah, $Eg)) $yb = iconv("utf-16", "utf-8", $yb);
            elseif ($Ah == "\xEF\xBB\xBF") $yb = substr($yb, 3);
            $I .= $yb . "\n\n";
        }
        else $I .= $yb;
    }
...
{
    return str_repeat("$Xf{0,65535}", $re / 65535) . "$Xf{0," . ($re % 65535) . "}";
}
function is_utf8($X)
{
...
function hidden_fields($ng, $Dd = array())
{
    $I = false;
    while (list($z, $X) = each($ng))
    {
...
function input($p, $Y, $s)
{
    ...
    if ($Ig && !$_POST["save"]) $s = null;
    $id = (isset($_GET["select"]) || $Ig ? array(
        "orig" => 'original'
    ) : array()) + $b->editFunctions($p);
    $Ja = " name='fields[$C]'";
    if ($p["type"] == "enum") echo h($id[""]) . "<td>" . $b->editInput($_GET["edit"], $p, $Ja, $Y);
    else
    {
        ...
        if ($Qd != "") echo $Qd;
        elseif (preg_match('~bool~', $p["type"])) echo "<input type='hidden'$Ja value='0'>" . "<input type='checkbox'" . (preg_match('~^(1|t|true|y|yes|on)$~i', $Y) ? " checked='checked'" : "") . "$Ja value='1'>";
        elseif ($p["type"] == "set")
        {
            preg_match_all("~'((?:[^']|'')*)'~", $p["length"], $Be);
            foreach ($Be[1] as $t => $X)
        ...
        {
            if ($ai && $y != "sqlite") $Ja .= " cols='50' rows='12'";
            else
            {
        ...
        else
        {
            ...
            if ($y == 'sql' && min_version(5.6) && preg_match('~time~', $p["type"])) $Ie += 7;
...
function dump_headers($Ad, $Se = false)
{
    ...
    $Hf = $_POST["output"];
    if ($Hf != "text") header("Content-Disposition: attachment; filename=" . $b->dumpFilename($Ad) . ".$I" . ($Hf != "file" && !preg_match('~[^0-9a-z]~', $Hf) ? ".$Hf" : ""));
    session_write_close();
    ob_flush();
    flush();
    return $I;
...
function select_value($X, $A, $p, $bi)
{
    ...
    {
        $I = "";
        foreach ($X as $be => $W) $I .= "<tr>" . ($X != array_values($X) ? "<th>" . h($be) : "") . "<td>" . select_value($W, $A, $p, $bi);
    ...
    {
        if (!is_utf8($I)) $I = "\0";
        elseif ($bi != "" && is_shortable($p)) $I = shorten_utf8($I, max(0, +$bi));
        else $I = h($I);
    }
...
function is_mail($oc)
{
    ...
    $Xf = "$Ha+(\\.$Ha+)*@($bc?\\.)+$bc";
    return is_string($oc) && preg_match("(^$Xf(,\\s*$Xf)*\$)i", $oc);
}
function is_url($P)
{
    $bc = '[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';
    return preg_match("~^(https?)://($bc?\\.)+$bc(:\\d+)?(/.*)?(\\?.*)?(#.*)?\$~i", $P);
}
...
{
    return script("mixin(qsl('select, input'), {onmouseover: function (event) { helpMouseover.call(this, event, $rb, $nh) }, onmouseout: helpMouseout});", "");
}
function edit_form($a, $q, $J, $Hi)
{
    global $b, $y, $mi, $o;
    $Nh = $b->tableName(table_status1($a, true));
    page_header(($Hi ? 'Edit' : 'Insert') , $o, array(
        "select" => array(
            $a,
            $Nh
        )
...
';
    ...
    else
    {
        ...
        foreach ($q as $C => $p)
        {
            ...
            {
                $Qb = $p["default"];
                if ($p["type"] == "bit" && preg_match("~^b'([01]*)'\$~", $Qb, $Eg)) $Qb = $Eg[1];
            }
...
if ($_GET["script"] == "version")
{
    ...
    if ($gd) file_write_unlock($gd, serialize(array(
        "signature" => $_POST["signature"],
        "version" => $_POST["version"]
    )));
    exit;
}
global $b, $g, $n, $cc, $kc, $uc, $o, $id, $od, $ba, $Pd, $y, $ca, $ke, $nf, $Yf, $Fh, $td, $mi, $si, $U, $Gi, $ia;
...
class Min_SQL
{
    ...
    function select($Q, $L, $Z, $ld, $xf = array() , $_ = 1, $E = 0, $jg = false)
    {
        global $b, $y;
        $Wd = (count($ld) < count($L));
        $G = $b->selectQueryBuild($L, $Z, $ld, $xf, $_, $E);
        if (!$G) $G = "SELECT" . limit(($_GET["page"] != "last" && $_ != "" && $ld && $Wd && $y == "sql" ? "SQL_CALC_FOUND_ROWS " : "") . implode(", ", $L) . "\nFROM " . table($Q) , ($Z ? "\nWHERE " . implode(" AND ", $Z) : "") . ($ld && $Wd ? "\nGROUP BY " . implode(", ", $ld) : "") . ($xf ? "\nORDER BY " . implode(", ", $xf) : "") , ($_ != "" ? +$_ : null) , ($E ? $_ * $E : 0) , "\n");
        $Ah = microtime(true);
        $I = $this
            ->_conn
            ->query($G);
        if ($jg) echo $b->selectQuery($G, $Ah, !$I);
        return $I;
    }
    function delete($Q, $tg, $_ = 0)
    {
        $G = "FROM " . table($Q);
        return queries("DELETE" . ($_ ? limit1($Q, $G, $tg) : " $G$tg"));
    }
    function update($Q, $O, $tg, $_ = 0, $M = "\n")
    {
        $Si = array();
        foreach ($O as $z => $X) $Si[] = "$z = $X";
        $G = table($Q) . " SET$M" . implode(",$M", $Si);
        return queries("UPDATE" . ($_ ? limit1($Q, $G, $tg, $M) : " $G$tg"));
    }
    function insert($Q, $O)
    {
        return queries("INSERT INTO " . table($Q) . ($O ? " (" . implode(", ", array_keys($O)) . ")\nVALUES (" . implode(", ", $O) . ")" : " DEFAULT VALUES"));
    }
    function insertUpdate($Q, $K, $hg)
    {
        return false;
    }
    function begin()
    ...
    function value($X, $p)
    {
        return (method_exists($this->_conn, 'value') ? $this
            ->_conn
            ->value($X, $p) : (is_resource($X) ? stream_get_contents($X) : $X));
    }
    function quoteBinary($Vg)
    {
        return q($Vg);
    }
    function warnings()
...
if (isset($_GET["sqlite"]) || isset($_GET["sqlite2"]))
{
        ...
        if (isset($_GET["sqlite"]))
        {
            class Min_SQLite
            {
                ...
                function query($G)
                {
                    ...
                    elseif ($H->numColumns()) return new Min_Result($H);
        ...
        else
        {
            class Min_SQLite
            {
                ...
                function __construct($Sc)
                {
                    ...
                    $this->_link = new SQLiteDatabase($Sc);
                }
                function query($G, $Ai = false)
                {
            ...
            class Min_Result
            {
                ...
                function fetch_assoc()
                {
                    ...
                    if (!$J) return false;
                    $I = array();
                    foreach ($J as $z => $X) $I[($z[0] == '"' ? idf_unescape($z) : $z) ] = $X;
                    return $I;
                }
                function fetch_row()
                {
    ...
    function fields($Q)
    {
        ...
        foreach (get_rows("PRAGMA table_info(" . table($Q) . ")") as $J)
        {
            ...
            $I[$C] = array(
                ...
                "full_type" => $T,
                "default" => (preg_match("~'(.*)'~", $Qb, $B) ? str_replace("''", "'", $B[1]) : ($Qb == "NULL" ? null : $Qb)) ,
                "null" => !$J["notnull"],
                "privileges" => array(
                    "select" => 1,
                    "insert" => 1,
            ...
            if ($J["pk"])
            {
                if ($hg != "") $I[$hg]["auto_increment"] = false;
                elseif (preg_match('~^integer$~i', $T)) $I[$C]["auto_increment"] = true;
                $hg = $C;
            }
        }
    ...
    function create_database($m, $d)
    {
        ...
        if (!check_sqlite_name($m)) return false;
        try
        {
            $A = new Min_SQLite($m);
        }
    ...
    {
        global $g;
        if (!check_sqlite_name($C)) return false;
        $g->__construct(":memory:");
        $g->error = 'File exists.';
        return @rename(DB, $C);
    }
    ...
    function alter_table($Q, $C, $q, $ad, $tb, $sc, $d, $La, $Rf)
    {
        ...
        if (!$Mi)
        {
            ...
            if ($Q != $C && !queries("ALTER TABLE " . table($Q) . " RENAME TO " . table($C))) return false;
        }
        elseif (!recreate_table($Q, $C, $c, $Ff, $ad)) return false;
        if ($La) queries("UPDATE sqlite_sequence SET seq = $La WHERE name = " . q($C));
        return true;
    }
    function recreate_table($Q, $C, $q, $Ff, $ad, $x = array())
    {
        if ($Q != "")
        {
            if (!$q)
            ...
            foreach (foreign_keys($Q) as $ee => $r)
            {
                ...
                if (!isset($ad[" $ee"])) $ad[] = " " . format_foreign_key($r);
            }
            queries("BEGIN");
        }
        foreach ($q as $z => $p) $q[$z] = "  " . implode($p);
        $q = array_merge($q, array_filter($ad));
        if (!queries("CREATE TABLE " . table($Q != "" ? "adminer_$C" : $C) . " (\n" . implode(",\n", $q) . "\n)")) return false;
        if ($Q != "")
        {
            if ($Ff && !queries("INSERT INTO " . table("adminer_$C") . " (" . implode(", ", $Ff) . ") SELECT " . implode(", ", array_map('idf_escape', array_keys($Ff))) . " FROM " . table($Q))) return false;
            $yi = array();
            ...
            if (!queries("DROP TABLE " . table($Q))) return false;
            queries("ALTER TABLE " . table("adminer_$C") . " RENAME TO " . table($C));
            if (!alter_indexes($C, $x)) return false;
            foreach ($yi as $vi)
            {
                if (!queries($vi)) return false;
            }
            queries("COMMIT");
        }
        return true;
    ...
    function alter_indexes($Q, $c)
    {
        foreach ($c as $hg)
        {
            if ($hg[0] == "PRIMARY") return recreate_table($Q, $Q, array() , array() , array() , $c);
        }
    ...
    {
        return apply_queries("DROP VIEW", $Xi);
    }
    ...
    function trigger($C)
    {
        ...
        return array(
            ...
            "Event" => strtoupper($B[2]) . ($gf ? " OF" : "") ,
            "Of" => ($gf[0] == '`' || $gf[0] == '"' ? idf_unescape($gf) : $gf) ,
            "Trigger" => $C,
            "Statement" => $B[4],
        );
    }
    ...
    function support($Nc)
    {
        return preg_match('~^(columns|database|drop_col|dump|indexes|descidx|move_col|sql|status|table|trigger|variables|view|view_trigger)$~', $Nc);
    }
    $y = "sqlite";
    $U = array(
        "integer" => 0,
    ...
    class Min_Driver extends Min_SQL
    {
        function insertUpdate($Q, $K, $hg)
        {
            ...
            foreach ($K as $O)
            {
                ...
                {
                    $Hi[] = "$z = $X";
                    if (isset($hg[idf_unescape($z) ])) $Z[] = "$z = $X";
                }
                if (!(($Z && queries("UPDATE " . table($Q) . " SET " . implode(", ", $Hi) . " WHERE " . implode(" AND ", $Z)) && $g->affected_rows) || queries("INSERT INTO " . table($Q) . " (" . implode(", ", array_keys($O)) . ") VALUES (" . implode(", ", $O) . ")"))) return false;
            }
            return true;
        }
        function slowQuery($G, $di)
        {
            $this
                ->_conn
                ->query("SET statement_timeout = " . (1000 * $di));
            $this
                ->_conn->timeout = 1000 * $di;
            return $G;
        }
        function convertSearch($v, $X, $p)
        {
...
ORDER BY a.attnum") as $J)
    ...
    function indexes($Q, $h = null)
    {
        ...
        foreach (get_rows("SELECT relname, indisunique::int, indisprimary::int, indkey, indoption , (indpred IS NOT NULL)::int as indispartial FROM pg_index i, pg_class ci WHERE i.indrelid = $Oh AND ci.oid = i.indexrelid", $h) as $J)
        {
            ...
            $I[$Fg]["columns"] = array();
            foreach (explode(" ", $J["indkey"]) as $Ld) $I[$Fg]["columns"][] = $f[$Ld];
            $I[$Fg]["descs"] = array();
            foreach (explode(" ", $J["indoption"]) as $Md) $I[$Fg]["descs"][] = ($Md & 1 ? '1' : null);
            $I[$Fg]["lengths"] = array();
        }
        return $I;
    }
...
WHERE table_schema = current_schema() AND table_name = " . q($C)))
    ...
    function alter_table($Q, $C, $q, $ad, $tb, $sc, $d, $La, $Rf)
    {
        ...
        foreach ($q as $p)
        {
            ...
            else
            {
                ...
                if ($p[0] == "") $c[] = ($Q != "" ? "ADD " : "  ") . implode($X);
                else
                {
                    if ($e != $X[0]) $sg[] = "ALTER TABLE " . table($Q) . " RENAME $e TO $X[0]";
                    $c[] = "ALTER $e TYPE$X[1]";
                    if (!$X[6])
                    {
                ...
                if ($p[0] != "" || $Ri != "") $sg[] = "COMMENT ON COLUMN " . table($Q) . ".$X[0] IS " . ($Ri != "" ? substr($Ri, 9) : "''");
            }
        }
        $c = array_merge($c, $ad);
        if ($Q == "") array_unshift($sg, "CREATE TABLE " . table($C) . " (\n" . implode(",\n", $c) . "\n)");
        elseif ($c) array_unshift($sg, "ALTER TABLE " . table($Q) . "\n" . implode(",\n", $c));
        if ($Q != "" && $Q != $C) $sg[] = "ALTER TABLE " . table($Q) . " RENAME TO " . table($C);
        if ($Q != "" || $tb != "") $sg[] = "COMMENT ON TABLE " . table($C) . " IS " . q($tb);
        if ($La != "")
        {
        }
        foreach ($sg as $G)
        {
            if (!queries($G)) return false;
        }
        return true;
    }
    function alter_indexes($Q, $c)
    {
        ...
        foreach ($c as $X)
        {
            ...
            elseif ($X[2] == "DROP") $dc[] = idf_escape($X[1]);
            else $sg[] = "CREATE INDEX " . idf_escape($X[1] != "" ? $X[1] : uniqid($Q . "_")) . " ON " . table($Q) . " (" . implode(", ", $X[2]) . ")";
        }
        if ($i) array_unshift($sg, "ALTER TABLE " . table($Q) . implode(",", $i));
        if ($dc) array_unshift($sg, "DROP INDEX " . implode(", ", $dc));
        foreach ($sg as $G)
        {
            if (!queries($G)) return false;
        }
    ...
    function drop_tables($S)
    {
        ...
        {
            $Ch = table_status($Q);
            if (!queries("DROP " . strtoupper($Ch["Engine"]) . " " . table($Q))) return false;
        }
        return true;
    }
    function move_tables($S, $Xi, $Vh)
    {
        ...
        {
            $Ch = table_status($Q);
            if (!queries("ALTER " . strtoupper($Ch["Engine"]) . " " . table($Q) . " SET SCHEMA " . idf_escape($Vh))) return false;
        }
        return true;
    }
    ...
    {
        $I = array();
        foreach (get_rows("SELECT * FROM information_schema.triggers WHERE event_object_table = " . q($Q)) as $J) $I[$J["trigger_name"]] = array(
            $J["action_timing"],
...
ORDER BY SPECIFIC_NAME');
    ...
    {
        global $g;
        if (preg_match("~ rows=([0-9]+)~", $g->result("EXPLAIN SELECT * FROM " . idf_escape($R["Name"]) . ($Z ? " WHERE " . implode(" AND ", $Z) : "")) , $Eg)) return $Eg[1];
        return false;
    }
    function types()
    {
...
AND typelem = 0");
    ...
    function create_sql($Q, $La, $Gh)
    {
        ...
        foreach ($x as $Gd => $w)
        {
            ...
            {
                case 'UNIQUE':
                    $Ng[] = "CONSTRAINT " . idf_escape($Gd) . " UNIQUE (" . implode(', ', array_map('idf_escape', $w['columns'])) . ")";
                break;
                case 'PRIMARY':
                    $Ng[] = "CONSTRAINT " . idf_escape($Gd) . " PRIMARY KEY (" . implode(', ', array_map('idf_escape', $w['columns'])) . ")";
                break;
            }
        }
        foreach ($Xc as $Wc => $Vc) $Ng[] = "CONSTRAINT " . idf_escape($Wc) . " $Vc[definition] " . ($Vc['deferrable'] ? 'DEFERRABLE' : 'NOT DEFERRABLE');
    ...
    function support($Nc)
    {
        return preg_match('~^(database|table|columns|sql|indexes|descidx|comment|view|' . (min_version(9.3) ? 'materializedview|' : '') . 'scheme|routine|processlist|sequence|trigger|type|variables|drop_col|kill|dump)$~', $Nc);
    }
    ...
    if (extension_loaded("oci8"))
    {
        class Min_DB
        {
            ...
            function query($G, $Ai = false)
            {
                ...
                if ($I)
                {
                    if (oci_num_fields($H)) return new Min_Result($H);
            ...
            {
                $H = $this->query($G);
                if (!is_object($H) || !oci_fetch($H->_result)) return false;
...
ORDER BY uc.constraint_type, uic.column_position", $h) as $J)
    ...
    function alter_table($Q, $C, $q, $ad, $tb, $sc, $d, $La, $Rf)
    {
        ...
        foreach ($q as $p)
        {
            ...
            if ($X) $c[] = ($Q != "" ? ($p[0] != "" ? "MODIFY (" : "ADD (") : "  ") . implode($X) . ($Q != "" ? ")" : "");
            else $dc[] = idf_escape($p[0]);
        }
        if ($Q == "") return queries("CREATE TABLE " . table($C) . " (\n" . implode(",\n", $c) . "\n)");
...
AND c_src.TABLE_NAME = " . q($Q);
        foreach (get_rows($G) as $J) $I[$J['NAME']] = array(
            "db" => $J['DEST_DB'],
            "table" => $J['DEST_TABLE'],
            "source" => array(
                $J['SRC_COLUMN']
...
');
    ...
    function support($Nc)
    {
        return preg_match('~^(columns|database|drop_col|indexes|descidx|processlist|scheme|sql|status|table|variables|view|view_trigger)$~', $Nc);
    }
    $y = "oracle";
    $U = array();
    $Fh = array();
    ...
    if (extension_loaded("sqlsrv"))
    {
        class Min_DB
        {
            ...
            function store_result($H = null)
            {
                if (!$H) $H = $this->_result;
                if (!$H) return false;
                if (sqlsrv_field_metadata($H)) return new Min_Result($H);
    ...
    class Min_Driver extends Min_SQL
    {
        function insertUpdate($Q, $K, $hg)
        {
            foreach ($K as $O)
            {
                ...
                if (!queries("MERGE " . table($Q) . " USING (VALUES(" . implode(", ", $O) . ")) AS source (c" . implode(", c", range(1, count($O))) . ") ON " . implode(" AND ", $Z) . " WHEN MATCHED THEN UPDATE SET " . implode(", ", $Hi) . " WHEN NOT MATCHED THEN INSERT (" . implode(", ", array_keys($O)) . ") VALUES (" . implode(", ", $O) . ");")) return false;
            }
            return true;
        }
        function begin()
...
WHERE OBJECT_NAME(i.object_id) = " . q($Q) , $h) as $J)
    ...
    function alter_table($Q, $C, $q, $ad, $tb, $sc, $d, $La, $Rf)
    {
        ...
        if ($Q == "") return queries("CREATE TABLE " . table($C) . " (" . implode(",", (array)$c["ADD"]) . "\n)");
        if ($Q != $C) queries("EXEC sp_rename " . q(table($Q)) . ", " . q($C));
        if ($ad) $c[""] = $ad;
        foreach ($c as $z => $X)
        {
            if (!queries("ALTER TABLE " . idf_escape($C) . " $z" . implode(",", $X))) return false;
        }
        return true;
    }
    function alter_indexes($Q, $c)
    {
        ...
        foreach ($c as $X)
        {
            if ($X[2] == "DROP")
            {
                if ($X[0] == "PRIMARY") $dc[] = idf_escape($X[1]);
    ...
    function drop_views($Xi)
    {
        return queries("DROP VIEW " . implode(", ", array_map('table', $Xi)));
    }
    function drop_tables($S)
    {
    ...
    function move_tables($S, $Xi, $Vh)
    {
        return apply_queries("ALTER SCHEMA " . idf_escape($Vh) . " TRANSFER", array_merge($S, $Xi));
    }
    function trigger($C)
    {
        if ($C == "") return array();
...
WHERE sys1.xtype = 'TR' AND sys2.name = " . q($Q)) as $J) $I[$J["name"]] = array(
            $J["Timing"],
            $J["Event"]
        );
        return $I;
    ...
    function support($Nc)
    {
        return preg_match('~^(columns|database|drop_col|indexes|descidx|scheme|sql|table|trigger|view|view_trigger)$~', $Nc);
    }
    $y = "mssql";
    $U = array();
    $Fh = array();
...
ORDER BY r.RDB$FIELD_POSITION';
        $H = ibase_query($g->_link, $G);
        while ($J = ibase_fetch_assoc($H)) $I[trim($J['FIELD_NAME']) ] = array(
    ...
    function support($Nc)
    {
        return preg_match("~^(columns|sql|status|table)$~", $Nc);
    }
    $y = "firebird";
    $sf = array(
        "="
...
if (isset($_GET["simpledb"]))
{
    ...
    if (class_exists('SimpleXMLElement') && ini_bool('allow_url_fopen'))
    {
        ...
        class Min_Result
        {
            ...
            function __construct($H)
            {
                ...
                {
                    $J = array();
                    if ($Zd->Name != '') $J['itemName()'] = (string)$Zd->Name;
                    foreach ($Zd->Attribute as $Ia)
                    {
    ...
    {
        public $hg = "itemName()";
        function _chunkRequest($Cd, $va, $Lf, $Cc = array())
        {
            global $g;
            foreach (array_chunk($Cd, 25) as $hb)
            {
                ...
                foreach ($hb as $t => $u)
                {
                    ...
                    foreach ($Cc as $z => $X) $Mf["Item.$t.$z"] = $X;
                }
                if (!sdb_request($va, $Mf)) return false;
            }
            $g->affected_rows = count($Cd);
            return true;
        }
        function _extractIds($Q, $tg, $_)
        {
            $I = array();
            if (preg_match_all("~itemName\(\) = (('[^']*+')+)~", $tg, $Be)) $I = array_map('idf_unescape', $Be[1]);
            else
            {
                foreach (sdb_request_all('Select', 'Item', array(
                    'SelectExpression' => 'SELECT itemName() FROM ' . table($Q) . $tg . ($_ ? " LIMIT 1" : "")
                )) as $Zd) $I[] = $Zd->Name;
            }
            return $I;
        }
        ...
        function delete($Q, $tg, $_ = 0)
        {
            return $this->_chunkRequest($this->_extractIds($Q, $tg, $_) , 'BatchDeleteAttributes', array(
                'DomainName' => $Q
            ));
        }
        function update($Q, $O, $tg, $_ = 0, $M = "\n")
        {
            ...
            foreach ($O as $z => $X)
            {
                ...
                if ($X == "NULL" || ($u != "" && array(
                    $u
                ) != $Cd)) $Sb["Attribute." . count($Sb) . ".Name"] = $z;
                if ($X != "NULL")
                {
                    foreach ((array)$X as $be => $W)
                    {
                        $Rd["Attribute.$t.Name"] = $z;
                        $Rd["Attribute.$t.Value"] = (is_array($X) ? $W : idf_unescape($W));
                        if (!$be) $Rd["Attribute.$t.Replace"] = "true";
                        $t++;
                    }
                }
            }
            ...
            return (!$Rd || $this->_chunkRequest(($u != "" ? array(
                $u
            ) : $Cd) , 'BatchPutAttributes', $Lf, $Rd)) && (!$Sb || $this->_chunkRequest($Cd, 'BatchDeleteAttributes', $Lf, $Sb));
        }
        function insert($Q, $O)
        {
            $Lf = array(
            ...
            foreach ($O as $C => $Y)
            {
                ...
                {
                    $C = idf_unescape($C);
                    if ($C == "itemName()") $Lf["ItemName"] = idf_unescape($Y);
                    else
                    {
            ...
            return sdb_request('PutAttributes', $Lf);
        }
        function insertUpdate($Q, $K, $hg)
        {
            foreach ($K as $O)
            {
                if (!$this->update($Q, $O, "WHERE `itemName()` = " . q($O["`itemName()`"]))) return false;
            }
            return true;
        }
        function begin()
    ...
    function table_status($C = "", $Mc = false)
    {
        ...
        ) : tables_list()) as $Q => $T)
        {
            ...
            if (!$Mc)
            {
                $Oe = sdb_request('DomainMetadata', array(
                    'DomainName' => $Q
                ));
                if ($Oe)
                {
    ...
    function sdb_request($va, $Lf = array())
    {
        global $b, $g;
        ...
        ksort($Lf);
        $G = '';
        foreach ($Lf as $z => $X) $G .= '&' . rawurlencode($z) . '=' . rawurlencode($X);
        $G = str_replace('%7E', '~', substr($G, 1));
        $G .= "&Signature=" . urlencode(base64_encode(hmac('sha1', "POST\n" . preg_replace('~^https?://~', '', $zd) . "\n/\n$G", $ah, true)));
        @ini_set('track_errors', 1);
        $Rc = @file_get_contents((preg_match('~^https?://~', $zd) ? $zd : "http://$zd") , false, stream_context_create(array(
            'http' => array(
                'method' => 'POST',
                'content' => $G,
                'ignore_errors' => 1,
    ...
    function sdb_request_all($va, $Uh, $Lf = array() , $di = 0)
    {
        $I = array();
        ...
        do
        {
            ...
            if (!$kj) break;
            foreach ($kj->$Uh as $nc) $I[] = $nc;
            if ($_ && count($I) >= $_)
            {
                $_GET["next"] = $kj->NextToken;
                break;
            }
            if ($di && microtime(true) - $Ah > $di) return false;
            $Lf['NextToken'] = $kj->NextToken;
            if ($_) $Lf['SelectExpression'] = preg_replace('~\d+\s*$~', $_ - count($I) , $Lf['SelectExpression']);
        }
        while ($kj->NextToken);
        return $I;
    }
...
if (isset($_GET["mongo"]))
{
    ...
    if (class_exists('MongoDB'))
    {
        class Min_DB
        {
            ...
            function connect($Ii, $vf)
            {
                return @new MongoClient($Ii, $vf);
            }
            function query($G)
            {
                return false;
        ...
        class Min_Driver extends Min_SQL
        {
            ...
            function select($Q, $L, $Z, $ld, $xf = array() , $_ = 1, $E = 0, $jg = false)
            {
                ...
                return new Min_Result($this
                    ->_conn
                    ->_db
                    ->selectCollection($Q)->find(array() , $L)->sort($sh)->limit($_ != "" ? +$_ : 0)->skip($E * $_));
            }
            function insert($Q, $O)
            {
                try
        ...
        function get_databases($Yc)
        {
            ...
            $Ob = $g
                ->_link
                ->listDBs();
            foreach ($Ob['databases'] as $m) $I[] = $m['name'];
            return $I;
        }
        function count_tables($l)
        {
            global $g;
            $I = array();
            foreach ($l as $m) $I[$m] = count($g
                ->_link
                ->selectDB($m)->getCollectionNames(true));
            return $I;
        }
        function tables_list()
        {
        ...
        function indexes($Q, $h = null)
        {
            ...
            foreach ($g
                ->_db
                ->selectCollection($Q)->getIndexInfo() as $w)
            {
                $Vb = array();
                foreach ($w["key"] as $e => $T) $Vb[] = ($T == - 1 ? '1' : null);
                $I[$w["name"]] = array(
    ...
    elseif (class_exists('MongoDB\Driver\Manager'))
    {
            ...
            function __construct($H)
            {
                foreach ($H as $Zd)
                {
                    ...
                    foreach ($Zd as $z => $X)
                    {
                        ...
                        $J[$z] = (is_a($X, 'MongoDB\BSON\ObjectID') ? 'MongoDB\BSON\ObjectID("' . strval($X) . '")' : (is_a($X, 'MongoDB\BSON\UTCDatetime') ? $X->toDateTime()
        ...
        class Min_Driver extends Min_SQL
        {
            ...
            function select($Q, $L, $Z, $ld, $xf = array() , $_ = 1, $E = 0, $jg = false)
            {
                ...
                $Mg = $g
                    ->_link
                    ->executeQuery("$g->_db_name.$Q", $G);
                return new Min_Result($Mg);
            }
            function update($Q, $O, $tg, $_ = 0, $M = "\n")
            {
                global $g;
                ...
                if (count($Gg)) $Hi['$unset'] = $Gg;
                $Ya->update($Z, $Hi, array(
                    'upsert' => false
                ));
                $Mg = $g
                    ->_link
                    ->executeBulkWrite("$m.$Q", $Ya);
        ...
        function tables_list()
        {
            ...
            $Mg = $g
                ->_link
                ->executeCommand($g->_db_name, $rb);
            $pb = array();
            foreach ($Mg as $H) $pb[$H->name] = 'table';
            return $pb;
        }
        function drop_databases($l)
        {
        ...
        function sql_query_where_parser($tg)
        {
            ...
            $ij = explode(') OR (', $tg);
            $Z = array();
            foreach ($hj as $fj) $Z[] = trim($fj);
            if (count($ij) == 1) $ij = array();
            elseif (count($ij) > 1) $Z = array();
            return where_to_query($Z, $ij);
        }
        function where_to_query($dj = array() , $ej = array())
        {
            global $b;
            $Jb = array();
            foreach (array(
            ...
            ) as $T => $Z)
            {
                if (is_array($Z))
                {
                    foreach ($Z as $Fc)
                    {
                        ...
                        if ($T == 'and') $Jb['$and'][] = array(
                            $mb => array(
                                $qf => $X
                            )
                        );
                        elseif ($T == 'or') $Jb['$or'][] = array(
                            $mb => array(
                                $qf => $X
                            )
                        );
    ...
    function support($Nc)
    {
        return preg_match("~database|indexes|descidx~", $Nc);
    }
    function db_collation($m, $ob)
    {
    }
...
if (isset($_GET["elastic"]))
{
    ...
    if (function_exists('json_decode') && ini_bool('allow_url_fopen'))
    {
        class Min_DB
        {
            ...
            function rootQuery($Vf, $yb = array() , $Pe = 'GET')
            {
                @ini_set('track_errors', 1);
                $Rc = @file_get_contents("$this->_url/" . ltrim($Vf, '/') , false, stream_context_create(array(
                    'http' => array(
                        'method' => $Pe,
                        'content' => $yb === null ? $yb : json_encode($yb) ,
            ...
            function query($Vf, $yb = array() , $Pe = 'GET')
            {
    ...
    class Min_Driver extends Min_SQL
    {
        function select($Q, $L, $Z, $ld, $xf = array() , $_ = 1, $E = 0, $jg = false)
        {
            ...
            foreach ($Z as $X)
            {
                ...
                if ($mb == "_id") $Jb["query"]["ids"]["values"][] = $X;
                elseif ($mb . $X != "")
                {
                    $Yh = array(
                        "term" => array(
                    ...
                    if ($qf == "=") $Jb["query"]["filtered"]["filter"]["and"][] = $Yh;
                    else $Jb["query"]["filtered"]["query"]["bool"]["must"][] = $Yh;
                }
            }
            if ($Jb["query"] && !$Jb["query"]["filtered"]["query"] && !$Jb["query"]["ids"]) $Jb["query"]["filtered"]["query"] = array(
                "match_all" => array()
            );
            $Ah = microtime(true);
            $Zg = $this
                ->_conn
                ->query($G, $Jb);
            if ($jg) echo $b->selectQuery("$G: " . print_r($Jb, true) , $Ah, !$Zg);
            if (!$Zg) return false;
            $I = array();
            foreach ($Zg['hits']['hits'] as $yd)
            {
                ...
                {
                    $q = array();
                    foreach ($L as $z) $q[$z] = $yd['fields'][$z];
                }
                foreach ($q as $z => $X)
                {
                    if ($Jb["fields"]) $X = $X[0];
        ...
        function insert($T, $yg)
        {
            ...
            $Jg = $this
                ...
                ->query($G, $yg, 'POST');
            $this
                ->_conn->last_id = $Jg['_id'];
            return $Jg['created'];
        }
        function delete($T, $tg, $_ = 0)
        {
            $Cd = array();
            if (is_array($_GET["where"]) && $_GET["where"]["_id"]) $Cd[] = $_GET["where"]["_id"];
            if (is_array($_POST['check']))
            {
                foreach ($_POST['check'] as $cb)
                {
                    $Tf = preg_split('~ *= *~', $cb);
                    if (count($Tf) == 2) $Cd[] = trim($Tf[1]);
                }
            }
            $this
            ...
            foreach ($Cd as $u)
            {
                ...
                $Jg = $this
                    ->_conn
                    ->query($G, '{}', 'DELETE');
                if (is_array($Jg) && $Jg['found'] == true) $this
                    ->_conn->affected_rows++;
            }
            return $this
                ->_conn->affected_rows;
    ...
    function support($Nc)
    {
        return preg_match("~database|table|columns~", $Nc);
    }
    function logged_user()
    {
        global $b;
    ...
    function fields($Q)
    {
        ...
        {
            $ye = $H[$Q]['properties'];
            if (!$ye) $ye = $H[$g->_db]['mappings'][$Q]['properties'];
            if ($ye)
            {
                foreach ($ye as $C => $p)
                {
    ...
    function drop_tables($S)
    {
        global $g;
        $I = true;
        foreach ($S as $Q) $I = $I && $g->query(urlencode($Q) , array() , 'DELETE');
        return $I;
    }
    function last_id()
    {
...
$cc["clickhouse"] = "ClickHouse (alpha)";
if (isset($_GET["clickhouse"]))
{
    ...
    class Min_DB
    {
        ...
        {
            @ini_set('track_errors', 1);
            $Rc = @file_get_contents("$this->_url/?database=$m", false, stream_context_create(array(
                'http' => array(
                    'method' => 'POST',
    ...
    class Min_Driver extends Min_SQL
    {
        function delete($Q, $tg, $_ = 0)
        {
            return queries("ALTER TABLE " . table($Q) . " DELETE $tg");
        }
        function update($Q, $O, $tg, $_ = 0, $M = "\n")
        {
            $Si = array();
            foreach ($O as $z => $X) $Si[] = "$z = $X";
            $G = $M . implode(",$M", $Si);
            return queries("ALTER TABLE " . table($Q) . " UPDATE $G$tg");
        }
    }
    function idf_escape($v)
    {
...
if (!defined("DRIVER"))
{
    ...
    if (extension_loaded("mysqli"))
    {
        class Min_DB extends MySQLi
        {
            ...
            function connect($N = "", $V = "", $F = "", $k = null, $ag = null, $rh = null)
            {
                global $b;
                mysqli_report(MYSQLI_REPORT_OFF);
                list($zd, $ag) = explode(":", $N, 2);
                $_h = $b->connectSsl();
                if ($_h) $this->ssl_set($_h['key'], $_h['cert'], $_h['ca'], '', '');
            ...
            function set_charset($bb)
            {
                if (parent::set_charset($bb)) return true;
    ...
    elseif (extension_loaded("mysql") && !((ini_bool("sql.safe_mode") || ini_bool("mysql.allow_local_infile")) && extension_loaded("pdo_mysql")))
    {
        class Min_DB
        {
            ...
            function set_charset($bb)
            {
                ...
                {
                    if (mysql_set_charset($bb, $this->_link)) return true;
    ...
    elseif (extension_loaded("pdo_mysql"))
    {
        class Min_DB extends Min_PDO
        {
            ...
            function connect($N, $V, $F)
            {
                ...
                $_h = $b->connectSsl();
                if ($_h) $vf += array(
                    PDO::MYSQL_ATTR_SSL_KEY => $_h['key'],
                    PDO::MYSQL_ATTR_SSL_CERT => $_h['cert'],
                    PDO::MYSQL_ATTR_SSL_CA => $_h['ca'],
                );
    ...
    class Min_Driver extends Min_SQL
    {
        ...
        function insertUpdate($Q, $K, $hg)
        {
            ...
            $Si = array();
            foreach ($f as $z) $Si[$z] = "$z = VALUES($z)";
        ...
        function slowQuery($G, $di)
        {
            if (min_version('5.7.8', '10.1.2'))
            {
                if (preg_match('~MariaDB~', $this
                    ->_conn
                    ->server_info)) return "SET STATEMENT max_statement_time=$di FOR $G";
                elseif (preg_match('~^(SELECT\b)(.+)~is', $G, $B)) return "$B[1] /*+ MAX_EXECUTION_TIME(" . ($di * 1000) . ") */ $B[2]";
            }
        }
        function convertSearch($v, $X, $p)
        {
        ...
        function tableHelp($C)
        {
            $ze = preg_match('~MariaDB~', $this
                ->_conn
                ->server_info);
            if (information_schema(DB)) return strtolower(($ze ? "information-schema-$C-table/" : str_replace("_", "-", $C) . "-table.html"));
            if (DB == "mysql") return ($ze ? "mysql$C-table/" : "system-database.html");
        }
    }
    function idf_escape($v)
    {
    ...
    function db_collation($m, $ob)
    {
        ...
        if (preg_match('~ COLLATE ([^ ]+)~', $i, $B)) $I = $B[1];
        elseif (preg_match('~ CHARACTER SET ([^ ]+)~', $i, $B)) $I = $ob[$B[1]][-1];
        return $I;
    }
    function engines()
    {
    ...
    {
        $I = array();
        foreach ($l as $m) $I[$m] = count(get_vals("SHOW TABLES IN " . idf_escape($m)));
        return $I;
    }
    function table_status($C = "", $Mc = false)
    {
        ...
        if ($Db)
        {
            ...
            foreach ($Be as $B)
            {
                ...
                $I[idf_unescape($B[1]) ] = array(
                    ...
                    "source" => array_map('idf_unescape', $th[0]) ,
                    "target" => array_map('idf_unescape', $Vh[0]) ,
    ...
    function rename_database($C, $d)
    {
        ...
        {
            $Hg = array();
            foreach (tables_list() as $Q => $T) $Hg[] = table($Q) . " TO " . idf_escape($C) . "." . table($Q);
    ...
    {
        $c = array();
        foreach ($q as $p) $c[] = ($p[1] ? ($Q != "" ? ($p[0] != "" ? "CHANGE " . idf_escape($p[0]) : "ADD") : " ") . " " . implode($p[1]) . ($Q != "" ? $p[2] : "") : "DROP " . idf_escape($p[0]));
        $c = array_merge($c, $ad);
    ...
    {
        $Hg = array();
        foreach (array_merge($S, $Xi) as $Q) $Hg[] = table($Q) . " TO " . idf_escape($Vh) . "." . table($Q);
        return queries("RENAME TABLE " . implode(", ", $Hg));
    }
    function copy_tables($S, $Xi, $Vh)
    {
        ...
        foreach ($Xi as $Q)
        {
            $C = ($Vh == DB ? table("copy_$Q") : idf_escape($Vh) . "." . table($Q));
            $Wi = view($Q);
            if (!queries("CREATE VIEW $C AS $Wi[select]")) return false;
        }
        return true;
    }
    function trigger($C)
    ...
    {
        $I = array();
        foreach (get_rows("SHOW TRIGGERS LIKE " . q(addcslashes($Q, "%_\\"))) as $J) $I[$J["Trigger"]] = array(
            $J["Timing"],
            $J["Event"]
        );
        return $I;
    ...
    function routine($C, $T)
    {
        ...
        foreach ($Be as $Kf)
        {
            ...
            $q[] = array(
                "field" => $C,
                "type" => strtolower($Kf[5]) ,
                "length" => preg_replace_callback("~$uc~s", 'normalize_enum', $Kf[6]) ,
                "unsigned" => strtolower(preg_replace('~\s+~', ' ', trim("$Kf[8] $Kf[7]"))) ,
                "null" => 1,
                "full_type" => $Kf[4],
                "inout" => strtoupper($Kf[1]) ,
                "collation" => strtolower($Kf[9]) ,
            );
        }
...
class Adminer
{
    ...
    function loginFormField($C, $vd, $Y)
    {
        return $vd . $Y;
    }
    function login($we, $F)
    ...
    function tableName($Mh)
    {
        return h($Mh["Name"]);
    }
    function fieldName($p, $xf = 0)
    {
        return '<span title="' . h($p["full_type"]) . '">' . h($p["field"]) . '</span>';
    }
    function selectLinks($Mh, $O = "")
    {
        global $y, $n;
        echo '<p class="links">';
        $ue = array(
        ...
        if (support("table") || support("indexes")) $ue["table"] = 'Show structure';
        if (support("table"))
        {
            if (is_view($Mh)) $ue["view"] = 'Alter view';
            else $ue["create"] = 'Alter table';
        }
        if ($O !== null) $ue["edit"] = 'New item';
        $C = $Mh["Name"];
        foreach ($ue as $z => $X) echo " <a href='" . h(ME) . "$z=" . urlencode($C) . ($z == "edit" ? $O : "") . "'" . bold(isset($_GET[$z])) . ">$X</a>";
        echo doc_link(array(
            $y => $n->tableHelp($C)
        ) , "?") , "\n";
    }
    ...
    function selectQuery($G, $Ah, $Lc = false)
    {
        global $y, $n;
        $I = "</p>\n";
        if (!$Lc && ($aj = $n->warnings()))
    ...
    function selectVal($X, $A, $p, $Ef)
    {
        $I = ($X === null ? "<i>NULL</i>" : (preg_match("~char|binary|boolean~", $p["type"]) && !preg_match("~var~", $p["type"]) ? "<code>$X</code>" : $X));
        if (preg_match('~blob|bytea|raw|file~', $p["type"]) && !is_utf8($X)) $I = "<i>" . lang(array(
            '%d byte',
            '%d bytes'
        ) , strlen($Ef)) . "</i>";
    ...
    function tableIndexesPrint($x)
    {
        ...
        foreach ($x as $C => $w)
        {
            ksort($w["columns"]);
            $jg = array();
            foreach ($w["columns"] as $z => $X) $jg[] = "<i>" . h($X) . "</i>" . ($w["lengths"][$z] ? "(" . $w["lengths"][$z] . ")" : "") . ($w["descs"][$z] ? " DESC" : "");
    ...
    function selectColumnsPrint($L, $f)
    {
        ...
        foreach ($L as $z => $X)
        {
            ...
            echo "<div>" . ($id || $od ? "<select name='columns[$t][fun]'>" . optionlist(array(-1 => ""
            ) + array_filter(array(
                'Functions' => $id,
                'Aggregation' => $od
    ...
    function selectOrderPrint($xf, $f, $x)
    {
    ...
    function selectActionPrint($x)
    {
        ...
        {
            $Ib = reset($w["columns"]);
            if ($w["type"] != "FULLTEXT" && $Ib) $f[$Ib] = 1;
        }
        $f[""] = 1;
        foreach ($f as $z => $X) json_row($z);
    ...
    function selectEmailPrint($pc, $f)
    {
    }
    ...
    function selectSearchProcess($q, $x)
    {
        ...
        foreach ((array)$_GET["where"] as $z => $X)
        {
            if ("$X[col]$X[val]" != "" && in_array($X["op"], $this->operators))
            {
                ...
                elseif ($X["op"] == "LIKE %%") $ub = " LIKE " . $this->processInput($q[$X["col"]], "%$X[val]%");
                elseif ($X["op"] == "ILIKE %%") $ub = " ILIKE " . $this->processInput($q[$X["col"]], "%$X[val]%");
                elseif ($X["op"] == "FIND_IN_SET")
                {
                    $fg = "$X[op](" . q($X["val"]) . ", ";
                    $ub = ")";
                }
                elseif (!preg_match('~NULL$~', $X["op"])) $ub .= " " . $this->processInput($q[$X["col"]], $X["val"]);
                if ($X["col"] != "") $I[] = $fg . $n->convertSearch(idf_escape($X["col"]) , $X, $q[$X["col"]]) . $ub;
                else
                {
                    $qb = array();
                    foreach ($q as $C => $p)
                    {
                        if ((preg_match('~^[-\d.' . (preg_match('~IN$~', $X["op"]) ? ',' : '') . ']+$~', $X["val"]) || !preg_match('~' . number_type() . '|bit~', $p["type"])) && (!preg_match("~[\x80-\xFF]~", $X["val"]) || preg_match('~char|text|enum|set~', $p["type"]))) $qb[] = $fg . $n->convertSearch(idf_escape($C) , $X, $p) . $ub;
                    }
    ...
    function selectEmailProcess($Z, $bd)
    {
        return false;
    }
    function selectQueryBuild($L, $Z, $ld, $xf, $_, $E)
    {
        return "";
    }
    function messageQuery($G, $ci, $Lc = false)
    {
        global $y, $n;
        restart_session();
        $wd = & get_session("queries");
        if (!$wd[$_GET["db"]]) $wd[$_GET["db"]] = array();
    ...
    function editFunctions($p)
    {
        ...
        foreach ($kc as $z => $id)
        {
            if (!$z || (!isset($_GET["call"]) && (isset($_GET["select"]) || where($_GET))))
            {
                foreach ($id as $Xf => $X)
                {
                    if (!$Xf || preg_match("~$Xf~", $p["type"])) $I .= "/$X";
                }
    ...
    function dumpTable($Q, $Gh, $Yd = 0)
    {
        ...
        else
        {
            ...
            else $i = create_sql($Q, $_POST["auto_increment"], $Gh);
            set_utf8mb4($i);
            if ($Gh && $i)
            {
                if ($Gh == "DROP+CREATE" || $Yd == 1) echo "DROP " . ($Yd == 2 ? "VIEW" : "TABLE") . " IF EXISTS " . table($Q) . ";\n";
                if ($Yd == 1) $i = remove_definer($i);
                echo "$i;\n\n";
            }
        }
    }
    function dumpData($Q, $Gh, $G)
    {
        global $g, $y;
        ...
        if ($Gh)
        {
            if ($_POST["format"] == "sql")
            {
                if ($Gh == "TRUNCATE+INSERT") echo truncate_sql($Q) . ";\n";
                $q = fields($Q);
            }
            $H = $g->query($G, 1);
            if ($H)
            {
                ...
                while ($J = $H->$Oc())
                {
                    ...
                    {
                        if (!$Rd) $Rd = "INSERT INTO " . table($Q) . " (" . implode(", ", array_map('idf_escape', $fe)) . ") VALUES";
                        foreach ($J as $z => $X)
                        {
                            $p = $q[$z];
                        ...
                        $Vg = ($De ? "\n" : " ") . "(" . implode(",\t", $J) . ")";
                        if (!$Xa) $Xa = $Rd . $Vg;
                        elseif (strlen($Xa) + 4 + strlen($Vg) + strlen($Ih) < $De) $Xa .= ",$Vg";
                        else
                        {
                            echo $Xa . $Ih;
                            $Xa = $Rd . $Vg;
    ...
    function dumpFilename($Ad)
    {
        return friendly_url($Ad != "" ? $Ad : (SERVER != "" ? SERVER : "localhost"));
    }
    function dumpHeaders($Ad, $Se = false)
    {
        $Hf = $_POST["output"];
                ...
                foreach ($jh as $N => $Pi)
                {
                    foreach ($Pi as $V => $F)
                    {
                        if ($F !== null)
                        {
                            ...
                            foreach (($Ob ? array_keys($Ob) : array(
                                ""
                            )) as $m) echo "<li><a href='" . h(auth_url($Ui, $N, $V, $m)) . "'>($cc[$Ui]) " . h($V . ($N != "" ? "@" . $this->serverName($N) : "") . ($m != "" ? " - $m" : "")) . "</a>\n";
                        }
                    }
                }
            }
...
';
                ...
                {
                    $ue = array();
                    foreach ($S as $Q => $T) $ue[] = preg_quote($Q, '/');
...
';
        ...
        echo "<span title='" . 'database' . "'>" . 'DB' . "</span>: " . ($l ? "<select name='db'>" . optionlist(array(
            "" => ""
        ) + $l, DB) . "</select>$Mb" : "<input name='db' value='" . h(DB) . "' autocapitalize='off'>\n") , "<input type='submit' value='" . 'Use' . "'" . ($l ? " class='hidden'" : "") . ">\n";
        ...
        foreach ($S as $Q => $Ch)
        {
            ...
            if ($C != "")
            {
                echo '<li><a href="' . h(ME) . 'select=' . urlencode($Q) . '"' . bold($_GET["select"] == $Q || $_GET["edit"] == $Q, "select") . ">" . 'select' . "</a> ", (support("table") || support("indexes") ? '<a href="' . h(ME) . 'table=' . urlencode($Q) . '"' . bold(in_array($Q, array(
                    ...
                    $_GET["foreign"],
                    $_GET["trigger"]
                )) , (is_view($Ch) ? "view" : "structure")) . " title='" . 'Show structure' . "'>$C</a>" : "<span>$C</span>") . "\n";
            }
        }
        echo "</ul>\n";
    }
...
function page_header($fi, $o = "", $Wa = array() , $gi = "")
{
    global $ca, $ia, $b, $cc, $y;
    page_headers();
    if (is_ajax() && $o)
...
-----END PUBLIC KEY-----
";
        if (openssl_verify($Vi["version"], base64_decode($Vi["signature"]) , $qg) == 1) $_COOKIE["adminer_version"] = $Vi["version"];
    }
    echo '<script', nonce() , '>
...
';
    if ($Wa !== null)
    {
        ...
        else
        {
            ...
            if ($_GET["ns"] != "" || (DB != "" && is_array($Wa))) echo '<a href="' . h($A . "&db=" . urlencode(DB) . (support("scheme") ? "&ns=" : "")) . '">' . h(DB) . '</a> &raquo; ';
            if (is_array($Wa))
            {
                ...
                foreach ($Wa as $z => $X)
                {
                    ...
                    if ($Ub != "") echo "<a href='" . h(ME . "$z=") . urlencode(is_array($X) ? $X[0] : $X) . "'>$Ub</a> &raquo; ";
                }
            }
            echo "$fi\n";
        }
...
{
    $Vg = '';
    foreach ($W as $X) $Vg .= pack('V', $X);
    if ($Zi) return substr($Vg, 0, end($W));
    return $Vg;
}
...
{
    return int32((($mj >> 5 & 0x7FFFFFF) ^ $lj << 2) + (($lj >> 3 & 0x1FFFFFFF) ^ $mj << 4)) ^ int32(($Jh ^ $lj) + ($be ^ $mj));
}
...
$g = '';
$td = $_SESSION["token"];
if (!$td) $_SESSION["token"] = rand(1, 1e6);
$mi = get_token();
$Yf = array();
...
if ($Ka)
{
    ...
    if (count($_POST) == 1 || DRIVER != $Ui || SERVER != $N || $_GET["username"] !== $V || DB != $m) redirect(auth_url($Ui, $N, $V, $m));
}
elseif ($_POST["logout"])
{
    if ($td && !verify_token())
...
function unset_permanent()
{
    ...
    {
        list($Ui, $N, $V, $m) = array_map('base64_decode', explode("-", $z));
        if ($Ui == DRIVER && $N == SERVER && $V == $_GET["username"] && $m == DB) unset($Yf[$z]);
    }
    cookie("adminer_permanent", implode(" ", $Yf));
}
function auth_error($o)
{
    ...
    if (isset($_GET["username"]))
    {
        ...
        if (($_COOKIE[$kh] || $_GET[$kh]) && !$td) $o = 'Session expired, please login again.';
        else
        {
            restart_session();
            add_invalid_login();
    ...
    if (!$_COOKIE[$kh] && $_GET[$kh] && ini_bool("session.use_only_cookies")) $o = 'Session support must be enabled.';
...
if (isset($_GET["username"]))
{
    ...
    if (is_numeric($ag) && $ag < 1024) auth_error('Connecting to privileged ports is not allowed.');
    check_invalid_login();
    $g = connect();
    $n = new Min_Driver($g);
}
$we = null;
if (!is_object($g) || ($we = $b->login($_GET["username"], get_password())) !== true)
{
...
if ($Ka && $_POST["token"]) $_POST["token"] = $mi;
$o = '';
if ($_POST)
{
    if (!verify_token())
...
function select($H, $h = null, $_f = array() , $_ = 0)
{
    global $y;
    $ue = array();
    $x = array();
    ...
    for ($t = 0;(!$_ || $t < $_) && ($J = $H->fetch_row());$t++)
    {
        if (!$t)
        {
            ...
            for ($ae = 0;$ae < count($J);$ae++)
            {
                ...
                $yf = $p->orgname;
                $I[$p->table] = $zf;
                if ($_f && $y == "sql") $ue[$ae] = ($C == "table" ? "table=" : ($C == "possible_keys" ? "indexes=" : null));
                elseif ($zf != "")
                {
                    if (!isset($x[$zf]))
                    {
                ...
                if ($p->charsetnr == 63) $Ta[$ae] = true;
                $U[$ae] = $p->type;
                echo "<th" . ($zf != "" || $p->name != $yf ? " title='" . h(($zf != "" ? "$zf." : "") . $yf) . "'" : "") . ">" . h($C) . ($_f ? doc_link(array(
        ...
        foreach ($J as $z => $X)
        {
            ...
            elseif ($Ta[$z] && !is_utf8($X)) $X = "<i>" . lang(array(
                '%d byte',
                '%d bytes'
            ) , strlen($X)) . "</i>";
            else
            ...
            if (isset($ue[$z]) && !$f[$ue[$z]])
            {
                ...
                else
                {
                    ...
                    foreach ($x[$ue[$z]] as $mb => $ae) $A .= "&where" . urlencode("[" . bracket_escape($mb) . "]") . "=" . urlencode($J[$ae]);
                }
                $X = "<a href='" . h(ME . $A) . "'>$X</a>";
            }
            echo "<td>$X";
...
{
    return cookie("adminer_settings", http_build_query($mh + adminer_settings()));
}
...
function edit_type($z, $p, $ob, $cd = array() , $Jc = array())
{
    global $Fh, $U, $Gi, $nf;
    $T = $p["type"];
    echo '<td><select name="', h($z) , '[type]" class="type" aria-labelledby="label-type">';
    if ($T && !isset($U[$T]) && !isset($cd[$T]) && !in_array($T, $Jc)) $Jc[] = $T;
    if ($cd) $Fh['Foreign keys'] = $cd;
    echo optionlist(array_merge($Jc, $Fh) , $T) , '</select>
...
function edit_fields($q, $ob, $T = "TABLE", $cd = array())
{
    global $Pd;
    $q = array_values($q);
    echo '<thead><tr>
...
function process_fields(&$q)
{
    ...
    if ($_POST["up"])
    {
        ...
        foreach ($q as $z => $p)
        {
            ...
            {
                unset($q[$z]);
                array_splice($q, $le, 0, array(
                    $p
                ));
                break;
            }
            if (isset($p["field"])) $le = $D;
            $D++;
        }
    }
    elseif ($_POST["down"])
...
function grant($jd, $mg, $f, $mf)
{
    ...
    return queries("$jd " . preg_replace('~(GRANT OPTION)\([^)]*\)~', '\1', implode("$f, ", $mg) . $f) . $mf);
}
function drop_create($dc, $i, $ec, $Zh, $gc, $ve, $Me, $Ke, $Le, $jf, $Xe)
{
    if ($_POST["drop"]) query_redirect($dc, $ve, $Me);
    elseif ($jf == "") query_redirect($i, $ve, $Le);
    elseif ($jf != $Xe)
    {
        $Eb = queries($i);
    ...
    else queries_redirect($ve, $Ke, queries($Zh) && queries($gc) && queries($dc) && queries($i));
}
...
function create_routine($Rg, $J)
{
    ...
    foreach ($q as $p)
    {
        if ($p["field"] != "") $O[] = (preg_match("~^($Pd)\$~", $p["inout"]) ? "$p[inout] " : "") . idf_escape($p["field"]) . process_type($p, "CHARACTER SET");
    }
    $Rb = rtrim("\n$J[definition]", ";");
...
function tar_file($Sc, $ji)
{
    ...
    $gb = 8 * 32;
    for ($t = 0;$t < strlen($I);$t++) $gb += ord($I[$t]);
    $I .= sprintf("%06o", $gb) . "\0 ";
    echo $I, str_repeat("\0", 512 - strlen($I));
    $ji->send();
    echo str_repeat("\0", 511 - ($ji->size + 511) % 512);
}
...
function doc_link($Wf, $ai = "<sup>?</sup>")
{
    ...
    $Li = array(
        'sql' => "https://dev.mysql.com/doc/refman/$Vi/en/",
        'sqlite' => "https://www.sqlite.org/",
        'pgsql' => "https://www.postgresql.org/docs/$Vi/static/",
...
elseif (isset($_GET["table"]))
{
    ...
    $b->selectLinks($R);
    $tb = $R["Comment"];
    if ($tb != "") echo "<p class='nowrap'>" . 'Comment' . ": " . h($tb) . "\n";
    ...
    if (support(is_view($R) ? "view_trigger" : "trigger"))
    {
        ...
        {
            echo "<table cellspacing='0'>\n";
            foreach ($yi as $z => $X) echo "<tr valign='top'><td>" . h($X[0]) . "<td>" . h($X[1]) . "<th>" . h($z) . "<td><a href='" . h(ME . 'trigger=' . urlencode($a) . '&name=' . urlencode($z)) . "'>" . 'Alter' . "</a>\n";
            echo "</table>\n";
        }
...
elseif (isset($_GET["schema"]))
{
    ...
    foreach (table_status('', true) as $Q => $R)
    {
        ...
        foreach ($b->foreignKeys($Q) as $X)
        {
            ...
            {
                $ne = $Qa;
                if ($Ph[$Q][1] || $Ph[$X["table"]][1]) $ne = min(floatval($Ph[$Q][1]) , floatval($Ph[$X["table"]][1])) - 1;
                else $Qa -= .1;
                while ($pe[(string)$ne]) $ne -= .0001;
                $Xg[$Q]["references"][$X["table"]][(string)$ne] = array(
                    $X["source"],
                    $X["target"]
                );
...
";
        ...
        foreach ((array)$l as $m)
        {
            ...
            if ($g->select_db($m))
            {
                ...
                {
                    set_utf8mb4($i);
                    if ($Gh == "DROP+CREATE") echo "DROP DATABASE IF EXISTS " . idf_escape($m) . ";\n";
                    echo "$i;\n";
                }
                if ($Xd)
                {
                    if ($Gh) echo use_sql($m) . ";\n\n";
                    $Gf = "";
                    if ($_POST["routines"])
                    {
                        foreach (array(
                ...
                if ($_POST["table_style"] || $_POST["data_style"])
                {
                    ...
                    foreach (table_status('', true) as $C => $R)
                    {
                        ...
                        if ($Q || $Jb)
                        {
                            ...
                            if ($Xd && $_POST["triggers"] && $Q && ($yi = trigger_sql($C))) echo "\nDELIMITER ;;\n$yi\nDELIMITER ;\n";
                            if ($Gc == "tar")
                            {
                                ob_end_flush();
                    ...
                    foreach ($Xi as $Wi) $b->dumpTable($Wi, $_POST["table_style"], 1);
                    if ($Gc == "tar") echo pack("x512");
                }
            }
        }
        if ($Xd) echo "-- " . $g->result("SELECT NOW()") . "\n";
        exit;
    }
    page_header('Export', $o, ($_GET["export"] != "" ? array(
        "table" => $_GET["export"]
    ) : array()) , h(DB));
    echo '
...
', script("qsl('table').onclick = dumpClick;");
    ...
    if (DB != "")
    {
        ...
        foreach ($Sh as $C => $T)
        {
            ...
            if ($T !== null && !preg_match('~table~i', $T)) $Xi .= "$jg\n";
            else echo "$jg<td align='right'><label class='block'><span id='Rows-" . h($C) . "'></span>" . checkbox("data[]", $C, $eb) . "</label>\n";
            $gg[$fg]++;
        }
        echo $Xi;
        if ($Sh) echo script("ajaxSetHtml('" . js_escape(ME) . "script=db');");
    }
    else
    {
        ...
        if (is_string($G))
        {
            ...
            while ($G != "")
            {
                ...
                else
                {
                    ...
                    list($ed, $bg) = $B[0];
                    if (!$ed && $gd && !feof($gd)) $G .= fread($gd, 1e5);
                    else
                    {
                        if (!$ed && rtrim($G) == "") break;
                        $D = $bg + strlen($ed);
                        if ($ed && rtrim($ed) != $Tb)
                        {
                            ...
                            {
                                $Vg = $B[0][0];
                                if (!$Vg && $gd && !feof($gd)) $G .= fread($gd, 1e5);
                                else
                                {
                                    $D = $B[0][1] + strlen($Vg);
                                    if ($Vg[0] != "\\") break;
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                $Ah = microtime(true);
                                if ($g->multi_query($rg) && is_object($h) && preg_match("~^$uh*+USE\\b~i", $rg)) $h->query($rg);
                                do
                                {
                                    $H = $g->store_result();
                                    if ($g->error)
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            $_ = $_POST["limit"];
                                            $_f = select($H, $h, array() , $_);
                                            if (!$_POST["only_errors"])
                                            {
                                                echo "<form action='' method='post'>\n";
                                                $df = $H->num_rows;
                                                echo "<p>" . ($df ? ($_ && $df > $_ ? sprintf('%d / ', $_) : "") . lang(array(
                                                    '%d row',
                                                    '%d rows'
                                                ) , $df) : "") , $ci;
                                                if ($h && preg_match("~^($uh|\\()*+SELECT\\b~i", $rg) && ($Dc = explain($h, $rg))) echo ", <a href='#$Ec'>Explain</a>" . script("qsl('a').onclick = partial(toggle, '$Ec');", "");
                                                $u = "export-$sb";
            ...
            if ($rc) echo "<p class='message'>" . 'No commands to execute.' . "\n";
            elseif ($_POST["only_errors"])
            {
                echo "<p class='message'>" . lang(array(
                    '%d query executed OK.',
                    '%d queries executed OK.'
                ) , $sb - count($wc)) , " <span class='time'>(" . format_time($oi) . ")</span>\n";
            }
...
';
    ...
    {
        $rg = $_GET["sql"];
        if ($_POST) $rg = $_POST["query"];
        elseif ($_GET["history"] == "all") $rg = $wd;
        elseif ($_GET["history"] != "") $rg = $wd[$_GET["history"]][0];
        echo "<p>";
...
elseif (isset($_GET["edit"]))
{
    ...
    foreach ($q as $C => $p)
    {
        if (!isset($p["privileges"][$Hi ? "update" : "insert"]) || $b->fieldName($p) == "") unset($q[$C]);
    }
    if ($_POST && !$o && !isset($_GET["select"]))
    {
        $ve = $_POST["referer"];
        if ($_POST["insert"]) $ve = ($Hi ? null : $_SERVER["REQUEST_URI"]);
        elseif (!preg_match('~^.+&select=.+$~', $ve)) $ve = ME . "select=" . urlencode($a);
        $x = indexes($a);
        $Ci = unique_array($_GET["where"], $x);
        $ug = "\nWHERE $Z";
        if (isset($_POST["delete"])) queries_redirect($ve, 'Item has been deleted.', $n->delete($a, $ug, !$Ci));
        else
        {
            $O = array();
            foreach ($q as $C => $p)
...
elseif (isset($_GET["create"]))
{
    ...
    $cd = array();
    foreach ($Bg as $Nh => $p) $cd[str_replace("`", "``", $Nh) . "`" . str_replace("`", "``", $p["field"]) ] = $Nh;
    $Cf = array();
    $R = array();
    if ($a != "")
    {
    ...
    if ($_POST && !process_fields($J["fields"]) && !$o)
    {
        ...
        else
        {
            ...
            foreach ($J["fields"] as $z => $p)
            {
                ...
                if ($p["field"] != "")
                {
                    ...
                    if ($og != process_field($Bf, $Bf))
                    {
                        ...
                        if ($p["orig"] != "" || $Aa) $Mi = true;
                    }
                    if ($r !== null) $ad[idf_escape($p["field"]) ] = ($a != "" && $y != "sqlite" ? "ADD" : " ") . format_foreign_key(array(
                        'table' => $cd[$p["type"]],
                        'source' => array(
                            $p["field"]
                        ) ,
            ...
            elseif (support("partitioning") && preg_match("~partitioned~", $R["Create_options"])) $Rf .= "\nREMOVE PARTITIONING";
    ...
    page_header(($a != "" ? 'Alter table' : 'Create table') , $o, array(
        "table" => $a
    ) , h($a));
    if (!$_POST)
    {
...
';
        ...
        echo ($tc ? "<select name='Engine'>" . optionlist(array(
            "" => "(" . 'engine' . ")"
        ) + $tc, $J["Engine"]) . "</select>" . on_help("getTarget(event).value", 1) . script("qsl('select').onchange = helpClose;") : "") , ' ', ($ob && !preg_match("~sqlite|mssql~", $y) ? html_select("Collation", array(
            "" => "(" . 'collation' . ")"
        ) + $ob, $J["Collation"]) : "") , ' <input type="submit" value="Save">
';
    }
    echo '
';
...
', "<select name='partition_by'>" . optionlist(array(
            "" => ""
        ) + $Pf, $J["partition_by"]) . "</select>" . on_help("getTarget(event).value.replace(/./, 'PARTITION BY \$&')", 1) . script("qsl('select').onchange = partitionByChange;") , '(<input name="partition" value="', h($J["partition"]) , '">)
...
elseif (isset($_GET["indexes"]))
{
    ...
    if (preg_match('~MyISAM|M?aria' . (min_version(5.6, '10.0.5') ? '|InnoDB' : '') . '~i', $R["Engine"])) $Hd[] = "FULLTEXT";
    if (preg_match('~MyISAM|M?aria' . (min_version(5.7, '10.2.2') ? '|InnoDB' : '') . '~i', $R["Engine"])) $Hd[] = "SPATIAL";
    $x = indexes($a);
    $hg = array();
    if ($y == "mongo")
    {
    ...
    if ($_POST["add"])
    {
        ...
        $w = end($J["indexes"]);
        if ($w["type"] || array_filter($w["columns"], 'strlen')) $J["indexes"][] = array(
            "columns" => array(
                1 => ""
            )
        );
...
';
    if ($hg)
    {
        ...
        {
            echo select_input(" disabled", $q, $e) , "<label><input disabled type='checkbox'>" . 'descending' . "</label> ";
        }
        echo "<td><td>\n";
    }
    $ae = 1;
    foreach ($J["indexes"] as $w)
    {
        ...
        {
            echo "<tr><td>" . html_select("indexes[$ae][type]", array(-1 => ""
            ) + $Hd, $w["type"], ($ae == count($J["indexes"]) ? "indexesAddRow.call(this);" : 1) , "label-type") , "<td>";
            ksort($w["columns"]);
            $t = 1;
            foreach ($w["columns"] as $z => $e)
            {
...
elseif (isset($_GET["database"]))
{
        ...
        elseif (DB !== $C)
        {
            ...
            else
            {
                ...
                foreach ($l as $m)
                {
                    if (count($l) == 1 || $m != "")
                    {
                        if (!create_database($m, $J["collation"])) $Hh = false;
                        $le = $m;
                    }
                }
                restart_session();
    ...
    $C = DB;
    if ($_POST) $C = $J["name"];
    elseif (DB != "") $J["collation"] = db_collation(DB, $ob);
    elseif ($y == "sql")
    {
...
elseif (isset($_GET["scheme"]))
{
    ...
    if ($_POST && !$o)
    {
        ...
        else
        {
            $C = trim($J["name"]);
            $A .= urlencode($C);
            if ($_GET["ns"] == "") query_redirect("CREATE SCHEMA " . idf_escape($C) , $A, 'Schema has been created.');
            elseif ($_GET["ns"] != $C) query_redirect("ALTER SCHEMA " . idf_escape($_GET["ns"]) . " RENAME TO " . idf_escape($C) , $A, 'Schema has been altered.');
            else redirect($A);
        }
    }
...
elseif (isset($_GET["call"]))
{
    ...
    {
        if (substr($p["inout"], -3) == "OUT") $Gf[$t] = "@" . idf_escape($p["field"]) . " AS " . idf_escape($p["field"]);
        if (!$p["inout"] || substr($p["inout"], 0, 2) == "IN") $Fd[] = $t;
    }
    if (!$o && $_POST)
    {
        $Za = array();
        foreach ($Rg["fields"] as $z => $p)
        {
            if (in_array($z, $Fd))
            {
                $X = process_input($p);
                if ($X === false) $X = "''";
                if (isset($Gf[$z])) $g->query("SET @" . idf_escape($p["field"]) . " = $X");
            }
...
elseif (isset($_GET["foreign"]))
{
    ...
    if ($_POST && !$o && !$_POST["add"] && !$_POST["change"] && !$_POST["change-js"])
    {
        ...
        if ($y == "sqlite") queries_redirect($ve, $Je, recreate_table($a, $a, array() , array() , array(
        ...
        else
        {
            ...
            if ($_POST["drop"]) query_redirect($c . $dc, $ve, $Je);
            else
            {
    ...
    page_header('Foreign key', $o, array(
        "table" => $a
    ) , h($a));
    if ($_POST)
    {
...
';
        ...
        foreach ($J["source"] as $z => $X)
        {
            ...
            ) + $th, $X, ($ae == count($J["source"]) - 1 ? "foreignAddRow.call(this);" : 1) , "label-source") , "<td>" . html_select("target[" . (+$z) . "]", $Vh, $J["target"][$z], 1, "label-target");
            $ae++;
        }
        echo '</table>
<p>
ON DELETE: ', html_select("on_delete", array(-1 => ""
        ) + explode("|", $nf) , $J["on_delete"]) , ' ON UPDATE: ', html_select("on_update", array(-1 => ""
        ) + explode("|", $nf) , $J["on_update"]) , doc_link(array(
...
elseif (isset($_GET["view"]))
{
    ...
    if ($_POST && !$o)
    {
        ...
        if (!$_POST["drop"] && $a == $C && $y != "sqlite" && $T == "VIEW" && $Df == "VIEW") query_redirect(($y == "mssql" ? "ALTER" : "CREATE OR REPLACE") . " VIEW " . table($C) . $Ga, $ve, $Je);
        else
        {
    ...
    page_header(($a != "" ? 'Alter view' : 'Create view') , $o, array(
        "table" => $a
    ) , h($a));
    echo '
<form action="" method="post">
...
elseif (isset($_GET["procedure"]))
{
    ...
    if ($_POST && !process_fields($J["fields"]) && !$o)
    {
        ...
        drop_create("DROP $Rg " . routine_id($da, $Af) , create_routine($Rg, $J) , "DROP $Rg " . routine_id($J["name"], $J) , create_routine($Rg, array(
            "name" => $Xh
        ) + $J) , "DROP $Rg " . routine_id($Xh, $J) , substr(ME, 0, -1) , 'Routine has been dropped.', 'Routine has been altered.', 'Routine has been created.', $da, $J["name"]);
    }
...
';
    ...
    if (isset($_GET["function"]))
    {
        ...
        edit_type("returns", $J["returns"], $ob, array() , ($y == "pgsql" ? array(
            "void",
            "trigger"
        ) : array()));
    }
...
elseif (isset($_GET["sequence"]))
{
    ...
    if ($_POST && !$o)
    {
        ...
        elseif ($fa == "") query_redirect("CREATE SEQUENCE " . idf_escape($C) , $A, 'Sequence has been created.');
...
elseif (isset($_GET["type"]))
{
    ...
    if ($_POST && !$o)
    {
        ...
        else query_redirect("CREATE TYPE " . idf_escape(trim($J["name"])) . " $J[as]", $A, 'Type has been created.');
    }
...
elseif (isset($_GET["trigger"]))
{
    ...
    page_header(($C != "" ? 'Alter trigger' . ": " . h($C) : 'Create trigger') , $o, array(
        "table" => $a
    ));
    echo '
<form action="" method="post" id="form">
...
elseif (isset($_GET["user"]))
{
    ...
    {
        foreach (explode(",", ($J["Privilege"] == "Grant option" ? "" : $J["Context"])) as $_b) $mg[$_b][$J["Privilege"]] = $J["Comment"];
    }
    ...
    foreach (array(
        ...
        "Update",
        "References"
    ) as $X) $mg["Columns"][$X] = $mg["Tables"][$X];
    unset($mg["Server Admin"]["Usage"]);
    foreach ($mg["Tables"] as $z => $X) unset($mg["Databases"][$z]);
    $We = array();
    if ($_POST)
    {
    ...
    if (isset($_GET["host"]) && ($H = $g->query("SHOW GRANTS FOR " . q($ha) . "@" . q($_GET["host"]))))
    {
        while ($J = $H->fetch_row())
        {
            if (preg_match('~GRANT (.*) ON (.*) TO ~', $J[0], $B) && preg_match_all('~ *([^(,]*[^ ,(])( *\([^)]+\))?~', $B[1], $Be, PREG_SET_ORDER))
            {
                foreach ($Be as $X)
                {
                    if ($X[1] != "USAGE") $kd["$B[2]$X[2]"][$X[1]] = true;
                    if (preg_match('~ WITH GRANT OPTION~', $J[0])) $kd["$B[2]$X[2]"]["GRANT OPTION"] = true;
                }
            }
    ...
    if ($_POST && !$o)
    {
        ...
        if ($_POST["drop"]) query_redirect("DROP USER $lf", ME . "privileges=", 'User has been dropped.');
        else
        {
            ...
            if (!$o)
            {
                ...
                elseif ($Uf != $kf) queries("SET PASSWORD FOR $Ye = " . q($Uf));
            }
            if (!$o)
            {
                $Og = array();
                foreach ($We as $ff => $jd)
                {
                    if (isset($_GET["grant"])) $jd = array_filter($jd);
                    $jd = array_keys($jd);
                    if (isset($_GET["grant"])) $Og = array_diff(array_keys(array_filter($We[$ff], 'strlen')) , $jd);
                    elseif ($lf == $Ye)
                    {
            ...
            {
                if ($lf != $Ye) queries("DROP USER $lf");
                elseif (!isset($_GET["grant"]))
                {
                    foreach ($kd as $ff => $Og)
                    {
                        if (preg_match('~^(.+)(\(.*\))?$~U', $ff, $B)) grant("REVOKE", array_keys($Og) , $B[2], " ON $B[1] FROM $Ye");
                    }
                }
            }
            queries_redirect(ME . "privileges=", (isset($_GET["host"]) ? 'User has been altered.' : 'User has been created.') , !$o);
            if ($Eb) $g->query("DROP USER $Ye");
        }
    }
    page_header((isset($_GET["host"]) ? 'Username' . ": " . h("$ha@$_GET[host]") : 'Create user') , $o, array(
        "privileges" => array(
            '',
            'Privileges'
        )
    ...
    else
    {
        ...
        $J["pass"] = $kf;
        if ($kf != "") $J["hashed"] = true;
        $kd[(DB == "" || $kd ? "" : idf_escape(addcslashes(DB, "%_\\"))) . ".*"] = array();
    }
    echo '<form action="" method="post">
<table cellspacing="0" class="layout">
...
';
    if (!$J["hashed"]) echo script("typePassword(qs('#pass'));");
    echo checkbox("hashed", 1, $J["hashed"], 'Hashed', "typePassword(this.form['pass'], this.checked);") , '</table>
    ...
    ) as $_b => $Ub)
    {
        foreach ((array)$mg[$_b] as $lg => $tb)
        {
            ...
            foreach ($kd as $ff => $jd)
            {
                ...
                $Y = $jd[strtoupper($lg) ];
                if ($_b == "Server Admin" && $ff != (isset($kd["*.*"]) ? "*.*" : ".*")) echo "<td>";
                elseif (isset($_GET["grant"])) echo "<td><select name=$C><option><option value='1'" . ($Y ? " selected" : "") . ">" . 'Grant' . "<option value='0'" . ($Y == "0" ? " selected" : "") . ">" . 'Revoke' . "</select>";
                else
                {
...
', script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});");
    ...
    foreach (process_list() as $t => $J)
    {
        if (!$t)
        {
            ...
            foreach ($J as $z => $X) echo "<th>$z" . doc_link(array(
        ...
        foreach ($J as $z => $X) echo "<td>" . (($y == "sql" && $z == "Info" && preg_match("~Query|Killed~", $J["Command"]) && $X != "") || ($y == "pgsql" && $z == "current_query" && $X != "<IDLE>") || ($y == "oracle" && $z == "sql_text" && $X != "") ? "<code class='jush-$y'>" . shorten_utf8($X, 100, "</code>") . ' <a href="' . h(ME . ($J["db"] != "" ? "db=" . urlencode($J["db"]) . "&" : "") . "sql=" . urlencode($X)) . '">' . 'Clone' . '</a>' : h($X));
        echo "\n";
    }
    echo '</table>
</div>
...
elseif (isset($_GET["select"]))
{
    ...
    foreach ($q as $z => $p)
    {
        ...
        if (isset($p["privileges"]["select"]) && $C != "")
        {
            ...
            if (is_shortable($p)) $bi = $b->selectLengthProcess();
        }
        $Pg += $p["privileges"];
    }
    ...
    if ($_POST && !$o)
    {
        ...
        {
            $fb = array();
            foreach ($_POST["check"] as $cb) $fb[] = where_check($cb, $q);
            $gj[] = "((" . implode(") OR (", $fb) . "))";
        }
        ...
        if ($_POST["export"])
        {
            ...
            $nd = ($ld && $Wd ? "\nGROUP BY " . implode(", ", $ld) : "") . ($xf ? "\nORDER BY " . implode(", ", $xf) : "");
            if (!is_array($_POST["check"]) || $hg) $G = "SELECT $hd$gj$nd";
            else
            {
                $Bi = array();
                foreach ($_POST["check"] as $X) $Bi[] = "(SELECT" . limit($hd, "\nWHERE " . ($Z ? implode(" AND ", $Z) . " AND " : "") . where_check($X, $q) . $nd, 1) . ")";
        ...
        if (!$b->selectEmailProcess($Z, $cd))
        {
            if ($_POST["save"] || $_POST["delete"])
            {
                ...
                {
                    $me = last_id();
                    if ($me) $Je = sprintf('Item%s has been inserted.', " $me");
                }
            ...
            elseif (!$_POST["import"])
            {
                if (!$_POST["val"]) $o = 'Ctrl+click on a value to modify it.';
                else
                {
                    $H = true;
                    $za = 0;
            ...
            elseif (!is_string($Rc = get_file("csv_file", true))) $o = upload_error($Rc);
            elseif (!preg_match('~~u', $Rc)) $o = 'File must be in UTF-8 encoding.';
            else
            {
                ...
                foreach ($Be[0] as $z => $X)
                {
                    ...
                    {
                        $O = array();
                        foreach ($Ce[1] as $t => $mb) $O[idf_escape($qb[$t]) ] = ($mb == "" && $q[$qb[$t]]["null"] ? "NULL" : q(str_replace('""', '"', preg_replace('~^"|"$~', '', $mb))));
                        $K[] = $O;
                    }
                }
    ...
    if (isset($Pg["insert"]) || !support("table"))
    {
        ...
        {
            if ($cd[$X["col"]] && count($cd[$X["col"]]) == 1 && ($X["op"] == "=" || (!$X["op"] && !preg_match('~[_%]~', $X["val"])))) $O .= "&set" . urlencode("[" . bracket_escape($X["col"]) . "]") . "=" . urlencode($X["val"]);
        }
    }
    $b->selectLinks($R, $O);
    ...
    else
    {
        ...
        if (!$bh)
        {
            ...
            if ($Ab) $bh[] = substr($Ab, 2);
        }
        foreach ($L as $z => $X)
        {
            $p = $q[idf_unescape($X) ];
            if ($p && ($Ga = convert_field($p))) $bh[$z] = "$Ga AS $X";
        }
        if (!$Wd && $Fi)
        {
            foreach ($Fi as $z => $X)
        ...
        else
        {
            ...
            if ($_GET["page"] != "last" && $_ != "" && $ld && $Wd && $y == "sql") $fd = $g->result(" SELECT FOUND_ROWS()");
            ...
            else
            {
                ...
                if ($_GET["modify"])
                {
                    foreach ($K as $J)
                    {
                        foreach ($J as $z => $X) $se[$z] = max($se[$z], min(40, strlen(utf8_decode($X))));
                    }
                }
                ...
                foreach ($b->rowDescriptions($K, $cd) as $Ue => $J)
                {
                    ...
                    foreach ($J as $z => $X)
                    {
                        if (isset($Ve[$z]))
                        {
                            $p = $q[$z];
                            $X = $n->value($X, $p);
                            if ($X != "" && (!isset($qc[$z]) || $qc[$z] != "")) $qc[$z] = (is_mail($X) ? $Ve[$z] : "");
                            $A = "";
                            if (preg_match('~blob|bytea|raw|file~', $p["type"]) && $X != "") $A = ME . 'download=' . urlencode($a) . '&field=' . urlencode($z) . $Di;
                            if (!$A && $X !== null)
                            {
                                foreach ((array)$cd[$z] as $r)
                                {
                                    ...
                                    {
                                        $A = "";
                                        foreach ($r["source"] as $t => $th) $A .= where_link($t, $r["target"][$t], $K[$Ue][$th]);
                            ...
                            if ($z == "COUNT(*)")
                            {
                                ...
                                foreach ($Ci as $be => $W) $A .= where_link($t++, $be, $W);
                            }
            ...
            if (!is_ajax())
            {
                if ($K || $E)
                {
                    ...
                    if ($_GET["page"] != "last")
                    {
                        if ($_ == "" || (count($K) < $_ && ($K || !$E))) $fd = ($E ? $E * $_ : 0) + count($K);
                        elseif ($y != "sql" || !$Wd)
                        {
                            $fd = ($Wd ? false : found_rows($R, $Z));
                            if ($fd < max(1e4, 2 * ($E + 1) * $_)) $fd = reset(slow_query(count_rows($a, $Z, $Wd, $ld)));
                            else $_c = false;
                        }
                    }
                ...
                if ($K || $E)
                {
                    if ($Jf)
                    {
                        ...
                        if ($y != "simpledb")
                        {
                            ...
                            for ($t = max(1, $E - 4);$t < min($Ee, $E + 5);$t++) echo pagination($t, $E);
                            if ($Ee > 0)
                            {
                    ...
                    $Zb = ($_c ? "" : "~ ") . $fd;
                    echo checkbox("all", 1, 0, ($fd !== false ? ($_c ? "" : "~ ") . lang(array(
                        '%d row',
                        '%d rows'
                    ) , $fd) : "") , "var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$Zb' : checked); selectCount('selected2', this.checked || !checked ? '$Zb' : checked);") . "\n", "</fieldset>\n";
...
elseif (isset($_GET["script"]))
{
    ...
    if ($_GET["script"] == "db")
    {
        ...
        foreach ($Kh as $z => $X) json_row("sum-$z", format_number($X));
        json_row("");
    }
        ...
        if ($_GET["ns"] !== "")
        {
            ...
            else
            {
                ...
                echo "<div class='scrollable'>\n", "<table cellspacing='0' class='nowrap checkable'>\n", script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});") , '<thead><tr class="wrap">', '<td><input id="check-all" type="checkbox" class="jsonly">' . script("qs('#check-all').onclick = partial(formCheck, /^(tables|views)\[/);", "") , '<th>' . 'Table', '<td>' . 'Engine' . doc_link(array(
                    ...
                    'sql' => 'charset-charsets.html',
                    'mariadb' => 'supported-character-sets-and-collations/'
                )) , '<td>' . 'Data Length' . $ac, '<td>' . 'Index Length' . $ac, '<td>' . 'Data Free' . $ac, '<td>' . 'Auto Increment' . doc_link(array(
                    'sql' => 'example-auto-increment.html',
                    'mariadb' => 'auto_increment/'
                )) , '<td>' . 'Rows' . $ac, (support("comment") ? '<td>' . 'Comment' . $ac : '') , "</thead>\n";
                $S = 0;
                foreach ($Sh as $C => $T)
                {
                ...
                if (!information_schema(DB))
                {
                    ...
                    echo script("qsl('input').onclick = function () { selectCount('selected', formChecked(this, /^(tables|views)\[/));" . (support("table") ? " selectCount('selected2', formChecked(this, /^tables\[/) || $S);" : "") . " }") , "<input type='hidden' name='token' value='$mi'>\n", "</div></fieldset>\n", "</div></div>\n";
                }
                echo "</form>\n", script("tableCheck();");
            }
            ...
            if (support("sequence"))
            {
                ...
                if ($hh)
                {
                    ...
                    odd('');
                    foreach ($hh as $X) echo "<tr" . odd() . "><th><a href='" . h(ME) . "sequence=" . urlencode($X) . "'>" . h($X) . "</a>\n";
                    echo "</table>\n";
                }
            ...
            if (support("type"))
            {
                ...
                if ($Oi)
                {
                    echo "<table cellspacing='0'>\n", "<thead><tr><th>" . 'Name' . "</thead>\n";
                    odd('');
                    foreach ($Oi as $X) echo "<tr" . odd() . "><th><a href='" . h(ME) . "type=" . urlencode($X) . "'>" . h($X) . "</a>\n";
                    echo "</table>\n";
                }
            ...
            if (support("event"))
            {
                ...
                if ($K)
                {
                    ...
                    if ($yc && $yc != "ON") echo "<p class='error'><code class='jush-sqlset'>event_scheduler</code>: " . h($yc) . "\n";
                }
